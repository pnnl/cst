...
class OSWTSO(Federate):
    ...
    def run_market_loop(self, market):
        """ 
        ...
        current_time = self.markets[market].current_start_time

        while current_time < pd.to_datetime(end):
            print('\n\n\n\n\n')
            print('current time : ',current_time)
            print('\n\n\n\n\n')
            self.markets[market].clear_market()
            filename = os.path.join(outdir,
                     market+'_'+pyen.utils.ioutils.format_filename(str(current_time))+'.json')
...
outdir = '/Users/camp426/github/pyenergymarket/data_model_tests/realtime_out'
...
markets = {}
start = "2032-01-01 00:00:00"
end = "2032-01-01 01:00:00"
pyenconfig = {
    "time": {
        "datefrom": start, 
        "dateto": end,
...
#%%
solver = pyenconfig['solve_arguments']['solver']
osw = OSWTSO("WECC_market", market_timing, markets, solver=solver)
market = "rt_energy_market"
osw.run_market_loop(market)#, "rt_market_results_")
