...
class Gridlabd:
    ...
    def read_player_files(self):
        for player_name in self.model.get('player', {}).keys():
            old_player_path = None
            player_file = self.model['player'][player_name].get('file')
            if player_file is None:
                raise RuntimeError(f"Player {player_name} does not have a file path.")
            if Path(player_file).absolute().exists():
                old_player_path = Path(player_file).absolute()
            if (self.base_dir_path / player_file).absolute().exists():
                old_player_path = (self.base_dir_path / player_file).absolute()
            if old_player_path is None:  # if still none after checking several places, raise an error
                raise FileNotFoundError(f"Cannot find player {player_name} at {player_file}")
            self.player_data[player_file] = pd.read_csv(old_player_path, header=None)

    def write_player_files(self, base_dir=None):
        if base_dir is None:
            base_dir = self.base_dir_path
        for player_file in self.player_data.keys():
            self.player_data[player_file].to_csv(Path(base_dir) / Path(player_file), index=False, header=False)
    ...
    def run(self, helics_config=None):
        # 1. create temp directory
        with tempfile.TemporaryDirectory() as tempdir:
            tmp_dir = Path(tempdir)
            output_name = tmp_dir / 'system.glm'
            # 2. create helics config if provided
            if helics_config is not None:
                federate_name = helics_config.get("name", "gld")
                config_path = tmp_dir / "glm_config.json"
                with open(config_path.absolute(), "w") as fp:
                    json.dump(helics_config, fp)
                self.add_helics(federate_name, str(config_path.absolute()))
            ...
            #         if old_player_path is None:  # if still none after checking several places, raise an error
            #             raise FileNotFoundError(f"Cannot find player {player_name} at {player_file}")
            #         shutil.copy(old_player_path, player_dir / old_player_path.name)
            #         self.model['player'][player_name]['file'] = str(Path(f"players/{old_player_path.name}"))
            # 3. Create subdirectory for output files to go to.
            player_dir = tmp_dir / "players"
            player_dir.mkdir()
            self.change_player_dirs("output")
            ...
            self.write_player_files(tmp_dir)
            # 5. Run glm file
    ...
    def change_player_dirs(self, new_player_dir):
        """
        Modify all the player file paths to have the path provided.

        Parameters
        ----------
        new_player_dir: str or Path -- directory where all player files will be stored.
        """
        for player_name in self.model.get('player', {}).keys():
            player_file = self.model['player'][player_name].get('file')
            if player_file is None:
                raise RuntimeError(f"Player {player_name} does not have a file path.")
            new_path = str(Path(f"{new_player_dir}/{Path(player_file).name}"))
            self.model['player'][player_name]['file'] = new_path
            self.player_data[new_path] = self.player_data[player_file]
            del self.player_data[player_file]

    def remove_outputs(self):
        """
        Delete all the objects that produce outputs.
