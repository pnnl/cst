...
RUN echo "===== Building CoSimulation Toolbox - Build =====" && \
  ...
  mkdir -p repo && \
#  mkdir -p tesp && \
  echo "$CST_USER" | sudo -S chown -hR $CST_USER:$CST_GRP ${CST_HOME} && \
  echo "$CST_USER" | sudo -S chmod -R g+rwX ${CST_HOME} && \
