...
def sample_ts_records():
    ...
    return [
        ...
        TSRecord(now + timedelta(seconds=60), 60.0, "TestScenario", "Battery", "Battery/voltage", 119.8),
...
class TestCSVTimeSeriesWriter:
    ...
    def test_writer_initialization(self, temp_directory):
        ...
        writer = CSVTimeSeriesWriter(location=temp_directory, analysis_name="test_analysis")
        assert isinstance(writer.helper, _CSVHelper)
        assert writer.helper.base_path == Path(temp_directory)
    ...
    def test_writer_connection(self, temp_directory):
        ...
        with writer:
            ...
            assert writer.helper.base_path.exists()
    ...
    def test_write_records(self, temp_directory, sample_ts_records):
        ...
        writer = CSVTimeSeriesWriter(location=temp_directory, analysis_name="TestAnalysis")
        with writer:
...
class TestCSVTimeSeriesReader:
    ...
    def test_reader_initialization(self, temp_directory):
        ...
        reader = CSVTimeSeriesReader(location=temp_directory, analysis_name="test_analysis")
    ...
    def test_read_data_empty(self, temp_directory):
        ...
        reader = CSVTimeSeriesReader(location=temp_directory, analysis_name="nonexistent")
        with reader:
            df = reader.read_data()
            assert df.empty
    ...
    def test_list_utilities_empty(self, temp_directory):
        ...
        reader = CSVTimeSeriesReader(location=temp_directory, analysis_name="empty_analysis")
        with reader:
...
class TestCSVTimeSeriesManager:
    ...
    def test_manager_initialization(self, temp_directory):
        ...
        manager = CSVTimeSeriesManager(location=temp_directory, analysis_name="TestAnalysis")
        assert manager.helper.base_path == Path(temp_directory)
